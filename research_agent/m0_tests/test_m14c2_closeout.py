...
def test_t215_promotion_fileset_no_forbidden_baggage():
    ...
    # The audit branch classifies every changed file as include | retain; a clean
    # promotion candidate carries ONLY the include files (retain paths absent).
    # Tracked retain files must appear in the diff; untracked/absent retain files
    # are legitimately not part of the candidate.
    ...
    tracked_res = subprocess.run(['git', 'ls-files'], cwd=ROOT, capture_output=True, text=True)
    assert tracked_res.returncode == 0, 'Git ls-files failed: %s' % (tracked_res.stderr or tracked_res.stdout)
    ...
    tracked_here = set(tracked_res.stdout.splitlines())
    retain_tracked = retain & tracked_here
    expected = include | retain_tracked
    assert changed_files == expected, (
        'Promotion classification must exactly equal canonical->HEAD diff for this branch '
        '(changed == include | retained-on-branch); missing=%s extra=%s' % (
            sorted(expected - changed_files), sorted(changed_files - expected)
        )
    )
    # Retain paths present on this branch must be part of the classified diff.
    for fpath in retain_tracked:
        assert fpath in changed_files, 'Tracked retain path %s must be in the classified diff' % fpath
    # Include paths must all differ from canonical on this branch.
    for fpath in include:
        assert fpath in changed_files, 'Include path %s must differ from canonical here' % fpath
