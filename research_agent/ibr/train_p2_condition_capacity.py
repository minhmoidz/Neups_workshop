"""STEP 7C — P2 Utility-Constrained Atlas Normalization attacker driver.

Reuses the exact STEP 7B pairwise condition-attacker design (same architecture class,
order-invariant |e1-e2|, BCE, Adam lr=1e-4, patience 5, min-val-BCE checkpointing).
Anatomy branch reads the P2 normalized cache (3x64x64 binary).

Arms:
    ARM A — P2 normalized anatomy only
    ARM B — pathology only (REUSES STEP 7A/7B records)
    ARM C — P2 normalized anatomy + pathology (JOINT PRIMARY)
...
OUT_DIR = 'research_agent/ibr_s1_condition_p2_capacity/'
...
def load_p2_cache():
    ...
    for fold in ('train', 'val'):
        p2 = np.load(os.path.join(P2_CACHE_DIR, '%s_p2.npy' % fold))
        with open(os.path.join(CACHE_DIR, '%s_images.json' % fold)) as f:
            images = json.load(f)
        assert p2.shape[0] == len(images)
        cache[fold] = {'p2': p2, 'images': images}
    pairs = {}
    for fold in ('train', 'val'):
...
class P2PairDataset(Dataset):
    """Pair row -> (m1, m2, y1, y2, label) with m = P2 normalized 3x64x64 uint8."""
    ...
    def __getitem__(self, i):
        a, b, lab = self.rows[i]
        m1 = self.cache['p2'][self.idx[a]].astype(np.float32)   # (3,64,64) binary
        m2 = self.cache['p2'][self.idx[b]].astype(np.float32)
        y1 = self.labels[a]
        y2 = self.labels[b]
...
def run_arm(arm, seed, device, cache, labels_by_image, pairs):
    ...
    tr = P2PairDataset('train', cache, labels_by_image, pairs)
    va = P2PairDataset('val', cache, labels_by_image, pairs)
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        if improved:
            ...
            torch.save({'arm': arm, 'seed': seed, 'epoch': ep, 'model': model.state_dict(),
                       ...
                       os.path.join(OUT_DIR, 'p2_arm%s_seed%d_best.pth' % (arm, seed)))
        else:
            patience += 1
        print('  p2 arm %s seed %d ep %02d tr %.4f va %.4f va_auc %.4f best %.4f@%d pat %d (%.0fs)'
...
def main():
    ...
    # ARM B (pathology only): reuse exact STEP 7A/7B records
