...
class Agent:
    def __init__(self, config):
        ...
        if self.generator_type == 'flow_field':
            ...
            # Direction-C: continuation init from an arbitrary generator
            # checkpoint given in the config (default keeps legacy path)
            _gen_init = self.config.get(
                'pretrained_generator_file',
                './networks/pretrained_generator_prichexy_net.pth')
            self.generator.load_state_dict(torch.load(_gen_init))
